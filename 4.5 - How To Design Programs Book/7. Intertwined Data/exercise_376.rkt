...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname exercise_376) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (enum-hello# xe)
  (local [(define content (xexpr-content xe))
          (define (sum-hellos fir res) (+ (item-hello# fir) res))]
    (foldr sum-hellos 0 content)))

(check-expect (enum-hello# x1) 1)
(check-expect (enum-hello# x2) 2)
(check-expect (enum-hello# x3) 0)

;XItem.v2 -> Number
; count the number of hellos
(define (item-hello# item)
  (local [(define content (first (xexpr-content item)))]
    (cond [(word? content) (hello-in-string (word-text content))]
          [else (+ 0 (enum-hello# content))])))

(define (hello-in-string str)
  (if (string=? str "hello") 1 0))

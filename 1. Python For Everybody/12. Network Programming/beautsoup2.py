...
# Actual problem: Start at: http: // py4e-data.dr-chuck.net/known_by_Natalia.html
...
# after a few seconds to make it difficult for you to d
# o the assignment without writing a Python program. 
# But frankly with a little effort and patience 
# you can overcome these attempts to make it a 
# little harder to complete the assignment without writing a Python program. 
...
href = 'http://py4e-data.dr-chuck.net/known_by_Nico.html'
for i in range(7):
    HTTPResponseObj = urllib.request.urlopen(href)
    html = HTTPResponseObj.read().decode()

    soup = BeautifulSoup(html, 'html.parser')

    link_list = soup.find_all('a')
    print(href)
    href = link_list[17].get('href')

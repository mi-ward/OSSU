...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
	  ...
	  | Shift(dX, dY, g_exp) => 
	  	(
		case eval_prog(g_exp, env) of
		Point (x, y) => eval_prog(Point(dX + x, dY + y), env)
		| Line(m, b) => eval_prog(Line(m, b + dY - (m * dX)), env)
		| VerticalLine(x) => eval_prog(VerticalLine(x + dX), env)
		| LineSegment(x1,y1,x2,y2) => eval_prog(LineSegment(x1+dX, y1+dY, x2+dX, y2+dY), env)
		| _ => eval_prog(g_exp, env)
		)
...
fun preprocess_prog (e) = 
	...
	LineSegment(x1,y1,x2,y2) => if real_close_point (x1,y1) (x2, y2)
								then intersect(Point(x1,y1), Point(x2,y2))
								else if real_close(x1,x2) andalso y2 < y1
									then LineSegment(x1,y2,x2,y1)
									else if x2 < x1 andalso real_close(y1,y2)
										then LineSegment(x2, y1, x1, y2)
										else if x2 < x1 andalso y2 < y1
